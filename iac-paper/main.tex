...
a precise apogee altitude and ensuring the deployment of the parachutes. This system consists of two parachutes and two redundant recovery avionics instruments which are used to estimate the apogee and trigger the separation of the rocket. After the separation occurs at the apogee, both parachutes are ejected from the body of the rocket and one is opened, the so-called drogue, reducing the speed of the rocket to about 30 m/s. The main parachute remains in a bag for the first part of the descend.
At apogee the nosecone is also separated from the rocket and the glider is ejected. From this point, the rocket consisting of thee parts (two body parts and the nosecone) and the glider descend independently of each other. At the altitude of 457 meters, the second (main) parachute opens, reducing the speed to 6m/s. Throughout the launch, separation and landing phases telemetry information, provided by the custom-designed avionics components placed in the nosecone of the rocket, are being constantly logged and part of them also sent to the ground station.

In the model flown at this year's competition no active aerodynamic control mechanisms were in place.
Therefore, once the rocket is launched, the trajectory cannot be influenced. This increased the importance of simulation, as it is the only way to determine the altitude the rocket can obtain with a chosen motor and given environmental conditions.  The trajectory of the rocket was simulated in three different simulators, including our own developed simulator \cite{sim_hassan}.
...
The second component of the project is the payload.
The payload is a glider equipped with an autopilot, Real Time Kinematic (RTK) GPS, telemetry link and control surfaces driven by servomotors.
...
The structure of this paper is as follows: in Section 2 the rocket is presented including the four main subsystems: structure, avionics, propulsion and recovery.
Section \ref{section:gliders} covers the design and technology used in a glider and presents a concept of a fleet of gliders.
Navigation and control, one of the main challenges in glider design, are elaborated in detail in Section \ref{subsection:navcontrol}.
The results, in terms of flight tests are analysed in Section \ref{sec:results}.
The last part contains the conclusion and the outlook.
