function log = extract_data ( state,t)
...
% the extraction of internal parameters
    ...
    env = environement(1400, 25, 85900, roro_data);
    %%
    ...
    for i = 2:ilast
         ...
         roro_data.time= t(i);
         burn_data(roro_data);
         roro_data.Xdot = state(i,1:3)' - state(i-1,1:3)';
         roro_data.Qdot = state(i,4:7)' - state(i-1,4:7)' ; 
         roro_data.Pdot = state(i,8:10)' - state(i-1,8:10)';
         roro_data.Ldot = state(i,11:13)' -state(i-1,11:13)';
        ...
        roro_data.alpha = alpha;
         ...
         log(i,1) = roro_data.X(3);
         log(i,2) = roro_data.Cd;
         log(i,3) = t(i);
    end
